...
TilleBar::TilleBar(QWidget *parent) : QWidget(parent)
{
...
#ifdef Q_OS_WINDOWS
            // 调用windows api 置顶层
            ::SetWindowPos((HWND)this->parentWidget()->winId(), HWND_TOPMOST, 0, 0, 0, 0,
...
#ifdef Q_OS_WINDOWS
            // 调用windows api 取消置顶层
            ::SetWindowPos((HWND)this->parentWidget()->winId(), HWND_NOTOPMOST, 0, 0, 0, 0,
                           SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
            pinBtn->setIcon(QIcon(":/image/ding1.png"));
#endif
        }
    });
    connect(minBtn, &QPushButton::clicked, this, [=]() { this->parentWidget()->showMinimized(); });
    connect(maxBtn, &QPushButton::clicked, this, [=]() {
        isMax = !isMax;
        // 最大化和复原
        if (isMax) {
            maxBtn->setIcon(QIcon(":/image/recover.png"));
            location = this->parentWidget()->geometry(); // 记录最大化之前的位置大小
            auto screens = QGuiApplication::screens();
            if (screens.count() > 1) {
                QScreen *currentScreen =
                    QGuiApplication::screenAt(this->parentWidget()->geometry().center());
                this->parentWidget()->setGeometry(currentScreen->availableGeometry());
            } else {
                this->parentWidget()->setGeometry(screens[0]->availableGeometry());
            }
        } else {
            maxBtn->setIcon(QIcon(":/image/max.png"));
    ...
    connect(closeBtn, &QPushButton::clicked, this, [=]() { this->parentWidget()->close(); });
}

TilleBar::~TilleBar() = default;
 ...
 * @param text 文本
 */
void TilleBar::setTille(const QString &text) { tille->setText(text); }
...
{
    if (isPressed) {
        QPointF mousePoint = event->globalPosition();
        if (isMax && !isDoubleClick) // 最大时拖动复原
        {
            ...
            maxBtn->click();
            this->parentWidget()->move(mousePoint.x() -
                                           (int)(proportion * this->parentWidget()->width()),
                                       mousePoint.y() - 16);
        } else // 拖动主窗口
        {
            QPointF movePoint = mousePoint - startMovePos;
