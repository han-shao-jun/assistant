...
QStringList SerialPortInfo::availablePorts()
{
    ...
    for (const auto &info : QSerialPortInfo::availablePorts()) {
        portList << info.portName().replace("COM", "");
    }

    // 使用标准库排序算法
    std::sort(portList.begin(), portList.end(),
              [](const QString &x, const QString &y) -> bool { return x.toInt() < y.toInt(); });

    for (auto &info : portList) {
        info.prepend("COM");
        auto temp = QSerialPortInfo(info).description();
        info.append(":").append(temp);
    }
